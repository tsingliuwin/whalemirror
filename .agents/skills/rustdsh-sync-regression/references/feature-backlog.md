...
| 5b | **存储格式 v4 升级** | 0.1.7-alpha.1 | web 已写 v4（互通阻断级）：SESSION_FORMAT_VERSION 4 + 读 v3/v4 + 迁移链 v0/v1→v2→v3→v4；V3→V4 语义=缺失 turn/end 补齐（observeRestart：turn/stepOpen/nextTurnSpliced 跟踪，reason=interrupted）+ subagent/catalog 新事件（finish 目录补齐去重）+ 消息源重写/内容迁移/retired syntax（15 子模块） | 迁移器大改；下轮主任务 | **待办（最优先）** |
